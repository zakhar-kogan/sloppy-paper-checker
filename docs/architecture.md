...
Public-beta inference controls are deliberately separate from report access: a per-browser rolling allowance limits ordinary use, a concurrency limit prevents overlapping runs, and an emergency switch disables new hosted analysis. Provider-side API-key limits protect operator spending. Existing compatible reports remain accessible when inference is unavailable.
